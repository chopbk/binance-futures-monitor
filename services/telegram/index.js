...
class TeleBot {
    ...
    sendReportCommand(message, cmdInfo) {
        this.bot.sendMessage(cmdInfo.chatId, message, { reply_to_message_id: cmdInfo.msgId });
        return;
    }
    run() {
        try {
            ...
            this.bot.on("channel_post", async (msg) => {
                ...
                if (this.id_command.includes(chatId.toString()) && messageText.startsWith("/")) {
                    this.eventEmitter.emit("new_command", messageText.slice(1), {
                        msgId: msg.message_id,
                        chatId: chatId,
                    });
            ...
            this.eventEmitter.addListener("report", (message, env) => {
                this.sendReport(`[${env || process.env.NODE_ENV}]: ${message}`);
            });
            this.eventEmitter.addListener("response_command", (message, env, cmdInfo) => {
                this.sendReportCommand(`[${env || process.env.NODE_ENV}]: ${message}`, cmdInfo);
            });
        } catch (error) {
            logger.error(`message + ${error}`);
        }
