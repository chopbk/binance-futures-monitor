...
function binanceFutures(options) {
  ...
  binance.futuresStopLimitShort = async (
    symbol,
    price,
    stoplossPrice,
    quantity = 0
  ...
  binance.futuresStopLimitLong = async (
    symbol,
    price,
    stoplossPrice,
    quantity = 0
  ...
  binance.futuresCancelAllOrdersOfSymbol = async (symbol, side) => {
    try {
      const allOpenOrders = await binance.futuresGetAllOrdersOfSymbol(
        symbol,
        side
      );
  ...
  // positions
  binance.futuresGetOpenPositionBySymbolAndSide = async (
    symbol = true,
    side = true
  ) => {
    try {
